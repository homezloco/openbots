...
- **MCP client** — a node with `"mcp"` in its tools and a configured server list can call tools on remote Streamable HTTP (or SSE) MCP servers you allowlist. The worker never spawns stdio processes and never loads user code. Operator fence: `ALLOWED_MCP_SERVERS` (empty-deny). See `docs/adapters.md`.
